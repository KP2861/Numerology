...
<body>
    ...
    <div class="container mt-5">
        ...
        @php
 //   dd(session('advance_numerology_data'));
@endphp
                    ...
                    <table class="table m-0">
                        <tbody>
                            @foreach ($result['Combinations'] as $combination => $data)
                                <tr class="{{ $data->type == 'Malefic' ? 'malefic-bg' : 'malefic-text blurred' }}">
                                    <td scope="row" class="py-2 px-5">{{ $combination }}</td>
                                    <td class="py-2 px-5 {{ $data->type == 'Malefic' ? 'malefic-bg' : 'malefic-text' }}">
                                        {{ $data->type }}
                                    </td>
                                </tr>
                            @endforeach
                ...
                <div class="message-section mt-4">
                    ...
                    <ul>
                        @foreach ($result['Combinations'] as $data)
                            <li>
                                <div class="message-box brown px-1 {{ $data->type == 'Malefic' ? 'malefic-bg' : 'malefic-text blurred' }}" style="font-size: 14px">
                                    {{ $data->message }}
                                </div>
                            </li>
                        @endforeach
